...
public class MarketplaceServiceTests
{
    ...
    public async Task CreateListing_Throws_WhenItemNotInInventory()
    {
        ...
        await Assert.ThrowsAsync<InvalidOperationException>(() =>
            service.CreateListingAsync(user.Id, new CreateMarketListingDTO { ItemId = weapon.Id, Price = 150 }));
    }
    ...
    public async Task GetWeapons_Sorts_ByPrice()
    {
        ...
        var service = CreateService(db, factory);
        var result = await service.GetWeaponsAsync(new WeaponQueryDTO { SortColumn = WeaponSortColumns.Price, SortDirection = SortDirection.Asc });
    ...
    public async Task GetWeapons_Pagination_Works()
    {
        ...
        var service = CreateService(db, factory);
        var page1 = await service.GetWeaponsAsync(new WeaponQueryDTO { PageNumber = 1, PageSize = 10 });
        var page2 = await service.GetWeaponsAsync(new WeaponQueryDTO { PageNumber = 2, PageSize = 10 });
    ...
    public async Task GetWeapons_Filters_ByName()
    {
        ...
        var service = CreateService(db, factory);
        var result = await service.GetWeaponsAsync(new WeaponQueryDTO { Search = "dragon" });
    ...
    public async Task GetWeapons_Throws_OnInvalidRange()
    {
        ...
        await Assert.ThrowsAsync<InvalidOperationException>(() =>
            service.GetWeaponsAsync(new WeaponQueryDTO { Cut = new RangeFilter<double> { Min = 100, Max = 10 } }));
    }
    ...
    public async Task GetArmors_Throws_OnInvalidRange()
    {
        ...
        await Assert.ThrowsAsync<InvalidOperationException>(() =>
            service.GetArmorsAsync(new ArmorQueryDTO { CutResistance = new RangeFilter<double> { Min = 100, Max = 10 } }));
    }
}